...
# Function to install a single package without using colorama
def install_package(package, pbar=None):
    try:
        if pbar:
            tqdm.write(f"Installing missing package: {package}...")
        else:
        ...
        if pbar:
            tqdm.write(f"Successfully installed {package}.\n")
        else:
            print(f"Successfully installed {package}.\n")
    except subprocess.CalledProcessError as e:
        if pbar:
            tqdm.write(f"[ERROR] Failed to install {package}: {e}")
        else:
            print(f"[ERROR] Failed to install {package}: {e}")
        sys.exit(1)

# Function to check and install required packages with a progress bar
def check_and_install_packages():
    required_packages = ['psutil', 'pyshark', 'colorama', 'tqdm']
    
    # Inform the user that the script is checking for necessary libraries
    print("Checking for necessary libraries...\n")
    
    # Total steps for the progress bar
    total_steps = len(required_packages)
    
    # Initialize tqdm progress bar
    try:
        from tqdm import tqdm
    except ImportError:
        # If tqdm is not installed, install it first
        subprocess.check_call([sys.executable, "-m", "pip", "install", "tqdm"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        from tqdm import tqdm

    # Import colorama early to use colored outputs in tqdm.write()
    ...
    with tqdm(total=total_steps, desc="Setting Up", bar_format="{l_bar}{bar} | {n_fmt}/{total_fmt} Steps") as pbar:
        for package in required_packages:
            if package == 'colorama' or package == 'tqdm':
                if importlib.util.find_spec(package) is None:
                    install_package(package, pbar)
                else:
                    tqdm.write(f"Library '{package}' is already installed.\n")
            else:
                if importlib.util.find_spec(package) is None:
                    tqdm.write(f"{Fore.CYAN}[INFO]{Style.RESET_ALL} Installing missing package: {package}")
                    install_package(package, pbar)
                else:
                    tqdm.write(f"Library '{package}' is already installed.\n")
            pbar.update(1)
    
    tqdm.write("All necessary libraries are installed.\n")
    ...
    return colorama, Fore, Style

# Call the package check and installation function
colorama, Fore, Style = check_and_install_packages()
...
def get_top_cpu_processes(n=5):
    ...
    # Initialize CPU percent using daemon threads
    threads = []
    for proc in exe_procs:
        thread = threading.Thread(target=init_cpu, args=(proc,), daemon=True)
        thread.start()
...
def monitor_udp_traffic(pid_ports, interface_name, duration):
    ...
    try:
        ...
        # Handle Ctrl+C during packet capture
        print("\n[WARNING] Script terminated by user.")
        capture.close()
        sys.exit(0)
    except Exception as e:
...
def main():
    # No initial console clearing to keep dependency messages visible
    # clear_console()  # Removed to keep the messages
...
if __name__ == "__main__":
    # Handle Ctrl+C gracefully in the main thread
    try:
        main()
    except KeyboardInterrupt:
        print("\n[WARNING] Script terminated by user.")
